...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .addField('\n\u200b\nInfórmáció:','**Szerver IP**: 185.249.198.6:30766 **Discord**: https://discord.gg/epQS62P Amennyiben problémába ütköztél nézz utána a segítségkérés szobában. Ha nem találsz rá megoldást, akkor keress fel egy **Admint!** YouTube csatornánk: **[Cloud RolePlay](https://www.youtube.com/channel/UCETy-Kw3Qbpid7Uu3zCKPZA?)**\n\u200b\n',false)
    if (STATUS !== undefined)
    {
    ...
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        if (players.length > 0) {
          ...
          // }
          fields[0] = `**A szerveren tartozkodó játékosok:**\n`;
          for (var i=0;i<players.length;i++) {
  ...
  bot.on('ready',() => {
    log(LOG_LEVELS.INFO,'Elindult...');
    // bot.user.setGame('Roofstad', '');
    ...
    return new Promise((resolve,reject) => {
      ...
      let id = loop_callbacks.push(() => {
        if (!resolved) {
          ...
          log(LOG_LEVELS.ERROR,'Nem volt nete');
          reject(null);
        }
      })
      setTimeout(() => {
  ...
  bot.on('reconnecting',(shard) => {
    log(LOG_LEVELS.INFO,'Újracsatlakozik');
    checkLoop().then((running) => {
      log(LOG_LEVELS.INFO,`Loop still running: ${running}`);
    }).catch(console.error);
  })
    ...
    if (!message.author.bot) {
      if (message.member) {
        ...
        if (message.channel.id === BUG_CHANNEL) {
          ...
          .setColor(0x2894C2)
          .setTitle('Bug Report')
          .setDescription('Elküldve az Adminoknak!')
          .setTimestamp(new Date());
